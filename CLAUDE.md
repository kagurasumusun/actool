...
Done: **dim2(9) key column** (`car::compute_keyformat_ios` inserts 9 then 8 after Subtype — iOS order is reverse of macOS's `…8,9…`) and the **iOS size→index map** (`catalog::ios_icon_dim2` / `icon_size_index`): 20→1, 29→2, 40→3, 60→4, 76→5, 83(.5)→6, 90→7, 1024→8. This fixed a real collision (same idiom+scale icons previously shared one key and collapsed). Multisize entries now carry correct indices.

Remaining:
- **dim1(8) key column** — comes from icon atlas packing (below); without packing our keyformat is `[…16,9,17…]` vs Apple's `[…16,9,8,17…]`.
- **Per-(idiom,subtype) MultiSized renditions**, each MSIS entry tagged with idiom. We still emit a single combined multisize. Reference emits: phone `{20:1,29:2,40:3,60:4}`, phone/**subtype 1792** `{90:7}`, pad `{20:1,29:2,40:3,76:5,83:6}`, marketing `{1024:8}`.
